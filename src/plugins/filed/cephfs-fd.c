...
static bRC pluginIO(bpContext *ctx, struct io_pkt *io)
{
   ...
   case IO_READ:
      if (p_ctx->cfd) {
         io->status = ceph_read(p_ctx->cmount, p_ctx->cfd, io->buf, io->count, -1);
         if (io->status < 0) {
   ...
   case IO_WRITE:
      if (p_ctx->cfd) {
         io->status = ceph_write(p_ctx->cmount, p_ctx->cfd, io->buf, io->count, -1);
         if (io->status < 0) {
            io->io_errno = -io->status;
            io->status = -1;
            goto bail_out;
   ...
   case IO_SEEK:
      if (p_ctx->cfd) {
         io->status = ceph_lseek(p_ctx->cmount, p_ctx->cfd, io->offset, io->whence);
         if (io->status < 0) {
            io->io_errno = -io->status;
            io->status = -1;
            goto bail_out;
...
static bRC setFileAttributes(bpContext *ctx, struct restore_pkt *rp)
{
    ...
    * Restore uid and gid.
    */
   status = ceph_lchown(p_ctx->cmount, rp->ofname, rp->statp.st_uid, rp->statp.st_gid);
   if (status < 0) {
      berrno be;
...
static bRC setXattr(bpContext *ctx, xattr_pkt *xp)
{
   ...
   status = ceph_lsetxattr(p_ctx->cmount, xp->fname, xp->name, xp->value, xp->value_length, 0);
   if (status < 0) {
      berrno be;
